...
import { generateQuestion, checkAnswer, recordAnswerResult, loadAdaptiveProgress, getAdaptiveInsights, TenseInsight, getPlayerBadge, PlayerBadge, getRecentAccuracyWindow } from './services/engine';
...
export default function SpanishTenseQuiz() {
  ...
  const renderInterface = () => {
    if (!currentQuestion) return <div className="p-8 text-center text-slate-400">Loading Quiz...</div>;
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center py-12 px-4">
      ...
      <header className="mb-8 w-full max-w-2xl flex flex-col items-center relative z-10">
        ...
        <div className="flex gap-2 w-full justify-center items-center">
            ...
            {Object.values(Difficulty).map((d) => (
                <button
                key={d}
                onClick={() => { setMode(d); nextQuestion(); setStreak(0); }}
        ...
        {isSettingsOpen && (
            <div className="absolute top-full mt-4 w-full bg-white p-6 rounded-2xl shadow-xl border border-slate-200 animate-in fade-in slide-in-from-top-2 z-20">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={toggleAllTenses} className="text-xs font-semibold text-indigo-600 hover:text-indigo-800 underline">
                        Toggle All
                    </button>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {Object.values(TenseId).map(t => (
                        ...
                        >
                            <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${selectedTenses.includes(t) ? 'border-indigo-500 bg-indigo-500' : 'border-slate-300'}`}>
                                {selectedTenses.includes(t) && <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path d="M5 13l4 4L19 7" /></svg>}
                            </div>
                ...
                <div className="mt-6 flex justify-end">
                    <button 
                        onClick={() => { setIsSettingsOpen(false); nextQuestion(); setStreak(0); }}
      ...
      {/* Main Card */}
      <main className={`w-full max-w-2xl mb-6 transition-opacity duration-200 ${isAnimating ? 'opacity-0' : 'opacity-100'}`}>
        {currentQuestion && (
            ...
            <div className="bg-slate-900 p-8 text-center relative">
                <div className="absolute top-4 right-4 flex items-center gap-2">
                    ...
                    <span className="bg-indigo-500 text-white px-2 py-0.5 rounded text-sm font-bold">{streak}</span>
                </div>
                ...
                <div className="flex flex-col gap-2 mb-2">
                    ...
                    <p className="text-slate-400 text-lg">
                        ...
                        <span className="text-slate-600 ml-2 italic">({currentQuestion.verb.translation})</span>
                    </p>
                </div>
            </div>
            ...
            <div className="p-8 bg-white min-h-[300px] flex flex-col justify-center">
                ...
                {/* Feedback Overlay */}
                {feedback && (
                <div className={`mt-6 p-4 rounded-xl text-center animate-in fade-in slide-in-from-bottom-4 ${
                    feedback.isCorrect ? 'bg-green-50 text-green-800 border border-green-200' : 'bg-red-50 text-red-800 border border-red-200'
                }`}>
                    <p className="font-bold text-lg mb-1">{feedback.isCorrect ? 'Correct!' : 'Wrong!'}</p>
                    <p>{feedback.message}</p>
                    {!feedback.isCorrect && (
                         <button onClick={nextQuestion} className="mt-3 px-4 py-2 bg-red-100 hover:bg-red-200 text-red-800 rounded-lg text-sm font-semibold">
                             Next Question
                         </button>
                    )}
                </div>
            ...
            <div className="bg-slate-50 p-4 border-t border-slate-100 flex justify-between items-center">
                <button onClick={skipQuestion} className="text-slate-400 hover:text-slate-600 text-sm font-medium px-4 py-2">
                    Skip Question
                </button>
                <div className="text-xs text-slate-300">
                    ID: {currentQuestion.id.split('-')[0]}
                </div>
            </div>
                                ...
                                {showMomentumPanel ? (
                                    recentSnapshot.sampleSize > 0 ? (
                                        <p className="text-sm text-slate-500">
                                            Recent accuracy {Math.round(recentSnapshot.accuracy * 100)}% · last {recentSnapshot.sampleSize}/{recentSnapshot.windowSize} attempts
                                        </p>
                                    ) : (
                                        <p className="text-sm text-slate-500">
                                    ...
                                    <p className="text-sm text-slate-500">Panel hidden · click show to bring it back.</p>
                                )}
                            </div>
                                    ...
                                    <div className="flex flex-col gap-2">
                                        {canExpandFocus && (
                                            <div className="flex justify-end">
                                                ...
                                                >
                                                    {showAllFocus ? 'Show less' : `Show all (${insightSummary.orderedFocus.length})`}
                                                </button>
                                            </div>
                                        )}
                                            ...
                                            return (
                                                <div key={entry.tense} className="bg-slate-50 rounded-xl border border-slate-100 p-4">
                                                    <div className="flex items-center justify-between">
                                                        ...
                                                        <p className="text-xs font-semibold text-indigo-500">Level {entry.level}</p>
                                                    </div>
                                                    ...
                                                    <div className="mt-2">
                                                        <div className="h-1.5 bg-white/70 rounded-full overflow-hidden">
                                                            <div
                                                                ...
                                                                style={{ width: `${Math.min(100, Math.round(entry.progress * 100))}%` }}
                                                            />
                                                        </div>
